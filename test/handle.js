...
const { action, handle } = require('..')

describe('handle', () => {
  const axn    = action('CREATE', 'blah')
  const bad    = action('BAD', 'blah')
  const create = spy()
  const res    = property()
  const update = spy()
  ...
  describe('when action type is found', () => {
    beforeEach(() =>
      app(axn).then(res)
    )

    it('invokes the correct handler', () => {
      expect(create.calls.length).to.equal(1)
      expect(create.calls[0]).to.eql([ axn ])
      expect(update.calls.length).to.equal(0)
    })
  })

  describe('when action type is not found', () => {
    beforeEach(() =>
      app(bad).catch(res)
    )

    it('rejects with an error accordingly', () => {
      expect(create.calls.length).to.equal(0)
      expect(update.calls.length).to.equal(0)
